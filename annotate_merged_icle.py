#!/usr/bin/env python3
"""
Annotate Merged ICLE Dataset with PassivePy and Custom Implementation

This script annotates the merged ICLE concordance dataset with both PassivePy and
...
try:
    ...
    print("Successfully imported PassivePy")
except ImportError:
...
def load_icle_data(filename='merged_icle_concordance.csv'):
    """Load merged ICLE concordance dataset"""
    print(f"Loading merged ICLE data from {filename}...")
    
    try:
        # Read the CSV file
        df = pd.read_csv(filename)
        print(f"Loaded {len(df)} records from the merged ICLE dataset")
        
        # If 'sentence' column doesn't exist, create it
        if 'sentence' not in df.columns:
            print("Creating 'sentence' column by combining Left, Center, and Right...")
...
def analyze_with_passivepy(df, sample_size=None):
    """Analyze text with PassivePy using correct initialization"""
    print("Analyzing with PassivePy...")
    
    # Take a sample if specified
    if sample_size and sample_size < len(df):
        df_sample = df.sample(n=sample_size, random_state=42)
        print(f"Using a sample of {len(df_sample)} sentences")
    else:
        df_sample = df
        print(f"Processing all {len(df_sample)} sentences")
    
    # Load the spaCy model first
    try:
        model_name = "en_core_web_sm"  # Use smaller model for faster processing
    ...
    # Initialize PassivePy with the loaded model
    passivepy = PassivePyAnalyzer(nlp=nlp)
    print("Successfully initialized PassivePyAnalyzer")
    ...
    for batch_idx in tqdm(range(num_batches), desc="Processing batches with PassivePy"):
        batch_start = batch_idx * batch_size
        batch_end = min((batch_idx + 1) * batch_size, len(df_sample))
        batch_df = df_sample.iloc[batch_start:batch_end]
        
        # Create a dataframe with just the text column for PassivePy
        text_df = pd.DataFrame({'text': batch_df['sentence'].values})
        
        try:
            # Process with PassivePy's match_corpus_level
            result_df = passivepy.match_corpus_level(
                text_df, 
                column_name='text',
                n_process=1,
                batch_size=batch_size // 4,  # Use smaller internal batches
            ...
            for i, row in result_df.iterrows():
                text = row['sentences']
                file_name = batch_df.iloc[i]['File name'] if 'File name' in batch_df.columns else f"doc_{batch_start + i}"
                native_language = batch_df.iloc[i]['Native language'] if 'Native language' in batch_df.columns else "Unknown"
                
                # Check for passive voice
                is_passive = row['binary'] == 1
                passive_count = row['all_passives_count']
                passive_phrases = row['all_passives'] if is_passive else []
            ...
            # Process sentences individually for this batch
            for i, row in batch_df.iterrows():
                text = row['sentence']
                file_name = row['File name'] if 'File name' in batch_df.columns else f"doc_{i}"
                native_language = row['Native language'] if 'Native language' in batch_df.columns else "Unknown"
                ...
                try:
                    ...
                    is_passive = passive_analysis["binary"].values[0] == 1
                    passive_count = passive_analysis["all_passives_count"].values[0]
                    passive_phrases = passive_analysis["all_passives"].values[0] if is_passive else []
...
def analyze_with_custom(df):
    ...
    # Log custom detector summary
    passive_count = sum(result_df['is_passive_custom'])
    print(f"Custom detector identified passive voice in {passive_count} out of {len(result_df)} sentences ({passive_count/len(result_df)*100:.1f}%)")
...
def calculate_metrics(df):
    ...
    # Precision, Recall, F1 (treating PassivePy as ground truth for consistency)
...
def visualize_metrics(metrics, output_file='icle_updated_comparison.png'):
    ...
    true_pos = metrics['comparison']['agreement'] - metrics['total_sentences'] + metrics['passivepy']['passive_sentences'] + metrics['custom']['passive_sentences']
    true_pos = true_pos // 2
    ...
    plt.subplot(2, 3, 4)
    languages = list(metrics['by_language'].keys())[:10]  # Top 10 languages
...
def save_metrics(metrics, filename='icle_updated_comparison.json'):
...
def main():
    ...
    merged_df = load_icle_data()
    if merged_df is None:
        print("Failed to load merged dataset. Exiting.")
        return
    ...
    passivepy_results.to_csv('icle_updated_passivepy_results.csv', index=False)
    print("Saved intermediate PassivePy results to icle_updated_passivepy_results.csv")
    
    # Analyze with custom detector
    comparison_results = analyze_with_custom(passivepy_results)
    
    # Save annotated dataset
    comparison_results.to_csv('icle_updated_annotated_comparison.csv', index=False)
    print("Saved annotated dataset to icle_updated_annotated_comparison.csv")
