#!/usr/bin/env python3
"""
Compare Simple PassivePy and Custom Passive Detector on ICLE Dataset

This script processes the ICLE dataset with both the simplified PassivePy implementation
and the custom passive detector, then compares their results.
"""
...
# Import our simple PassivePy implementation
from simple_passivepy import SimplePassivePyDetector
...
def analyze_with_simple_passivepy(df):
    """Analyze text with simplified PassivePy"""
    print("Analyzing with Simple PassivePy...")
    
    # Initialize the detector
    detector = SimplePassivePyDetector(spacy_model='en_core_web_sm')
    
    # Initialize results list
    results = []
    
    # Process each sentence
    for idx, row in tqdm(df.iterrows(), total=len(df), desc="Processing with Simple PassivePy"):
        text = row['sentence']
        ...
        # Process with Simple PassivePy
        try:
            # Get passive results
            passive_analysis = detector.detect_passive(text)
            
            # Check for passive voice
            is_passive = passive_analysis['is_passive']
            passive_count = passive_analysis['passive_count']
            passive_phrases = [p['span'] for p in passive_analysis['passive']]
            ...
            print(f"Error processing with Simple PassivePy: {e}")
            # Add empty results
            results.append({
                'text': text,
                'file_name': file_name,
...
def save_metrics(metrics, filename='simple_passivepy_custom_icle_comparison.json'):
...
def main():
    ...
    # Analyze with Simple PassivePy
    passivepy_results = analyze_with_simple_passivepy(sample_df)
    ...
    print(f"Simple PassivePy passive sentences: {metrics['passivepy']['passive_sentences']} ({metrics['passivepy']['passive_percentage']:.1f}%)")
    ...
    print(f"Simple PassivePy only: {metrics['comparison']['passivepy_only']}")
