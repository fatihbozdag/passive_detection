#!/usr/bin/env python3
"""
ICLE Anywords Concordance Analysis Script

This script processes the ICLE anywords concordance dataset, merges the Left, Center, and Right columns 
to create complete sentences, then analyzes them with our custom passive voice detector implementation.
Results are comparable with the original ICLE concordance analysis.
"""
...
def load_icle_concordance(file_path):
    """
    Load the ICLE anywords concordance dataset and create a unified sentence column
    ...
    """
    print(f"Loading ICLE anywords concordance data from {file_path}...")
    ...
    try:
        ...
        print(f"Loaded {len(df)} sentences from ICLE anywords concordance data")
        return df
...
def visualize_results(stats):
    ...
    plt.tight_layout()
    plt.savefig('icle_anywords_analysis.png', dpi=300, bbox_inches='tight')
    print("Visualizations saved to 'icle_anywords_analysis.png'")
    ...
    if 'language_stats' in stats and len(stats['language_stats']) > 10:
        ...
        plt.tight_layout()
        plt.savefig('icle_anywords_language_stats.png', dpi=300, bbox_inches='tight')
        print("Language statistics visualization saved to 'icle_anywords_language_stats.png'")

def main():
    """Main execution function"""
    # Load ICLE concordance data
    icle_file = 'icle_concord_anywords.csv'
    ...
    df.to_csv('icle_anywords_annotated.csv', index=False)
    print("Annotated dataset saved to 'icle_anywords_annotated.csv'")
    ...
    if stats and 'language_stats' in stats:
        stats['language_stats'].to_csv('icle_anywords_language_stats.csv')
        print("Language statistics saved to 'icle_anywords_language_stats.csv'")
